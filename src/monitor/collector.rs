...
fn compute_cpu_percents(prev: &CpuStatRaw, curr: &CpuStatRaw) -> (f32, f32, f32, f32) {
    ...
    (
        (d_user as f32 * s).min(100.0),
        (d_system as f32 * s).min(100.0),
        (d_idle as f32 * s).min(100.0),
        (d_steal as f32 * s).min(100.0),
    )
}
...
fn read_diskstats() -> HashMap<String, (u64, u64)> {
    ...
    let Ok(c) = std::fs::read_to_string("/proc/diskstats") else {
        return r;
    };
    for line in c.lines() {
        let p: Vec<&str> = line.split_whitespace().collect();
        ...
        r.insert(
            p[2].to_string(),
            (p[5].parse().unwrap_or(0), p[9].parse().unwrap_or(0)),
        );
    }
    r
}
...
fn compute_disk_io_rates(
    ...
    for (name, (sr_a, sw_a)) in after {
        if let Some((sr_b, sw_b)) = before.get(name) {
            ...
            read_rates.insert(
                name.clone(),
                (dr as f64 * SECTOR_BYTES / elapsed_secs).round() as u64,
            );
            write_rates.insert(
                name.clone(),
                (dw as f64 * SECTOR_BYTES / elapsed_secs).round() as u64,
            );
        }
    }
...
impl SystemMonitor {
    ...
    pub async fn update_fast_metrics(&mut self) {
        let refresh_latency =
            should_refresh_every(&mut self.latency_cycle, LATENCY_INTERVAL_CYCLES);
        let refresh_cpu_frequency =
            should_refresh_every(&mut self.cpu_frequency_cycle, CPU_FREQUENCY_INTERVAL_CYCLES);
    ...
    fn collect_sensor_metrics(&self) -> SensorMetrics {
        ...
        fn chip_matches(sensor: &TemperatureSensor, chips: &[&str]) -> bool {
            chips
                .iter()
                .any(|chip| sensor.chip.eq_ignore_ascii_case(chip))
        }

        let hottest = max_by_temp(temperatures.iter());
        let cpu_chips = ["coretemp", "k10temp", "zenpower"];
        let gpu_chips = ["amdgpu", "radeon", "nouveau"];
        let hottest_cpu = max_by_temp(temperatures.iter().filter(|s| chip_matches(s, &cpu_chips)));
        ...
        let hottest_gpu_label = hottest_gpu_chip
            .map(|s| s.label.clone())
            .unwrap_or_default();
